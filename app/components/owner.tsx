  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto animate-fade-in pb-12">
      <div className="flex justify-between items-center border-b border-[#3d2e24]/10 pb-4">
        <div>
          <h2 className="text-sm font-black text-[#3d2e24] uppercase tracking-widest">
            لوحة الإدارة المركزية والتحكم للمالك
          </h2>
          <p className="text-[11px] text-stone-400 mt-0.5 font-medium">
            مراقبة حية لإجماليات الأداء العام وجرد المخازن ووسائط براند ابن شالي
          </p>
        </div>
        <button
          onClick={() => setRole('cashier')}
          className="bg-[#3d2e24] hover:bg-[#1e6b65] text-white px-4 py-2 rounded-xl text-xs font-black transition"
        >
          الولوج لواجهة الكاشير ↩
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-2xl p-5 shadow-sm">
          <span className="text-[10px] text-stone-400 font-bold uppercase block">
            إجمالي مبيعات الخزنة الموحدة اليوم
          </span>
      ...
      <div className="bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-3xl p-6 shadow-md space-y-6">
        <div className="border-b border-stone-100 pb-2">
          <h3 className="text-xs font-black text-[#3d2e24] flex items-center gap-1.5">
            📸 إدارة وسائط المتجر وصورة الواجهة الخلفية
          </h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
          <div className="space-y-1">
            <label className="text-[11px] font-black block text-[#3d2e24]">
              تحديث صورة واجهة المتجر الخلفية (Hero Banner):
            </label>
            <p className="text-[10px] text-stone-400 font-medium leading-relaxed">
              ارفع هنا لقطة جبال وبحيرة واحة سيوة الصادقة عالية الدقة لتصبح الخلفية الرسمية الفورية لزوار المتجر.
            </p>
          </div>
        ...
        <div className="pt-4 border-t border-stone-100 space-y-2">
          <label className="text-[11px] font-black block text-[#3d2e24] mb-2">
            تحديث صور كروت المنتجات الفردية (Base64 السحابية):
          </label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {products.map(p => (
              <div
                key={p.id}
                className="bg-[#f5f2eb]/40 border border-[#3d2e24]/10 rounded-2xl p-4 flex justify-between items-center gap-2"
              >
                <span className="text-xs font-bold text-[#3d2e24] truncate max-w-[180px]">
                  {p.name}
                </span>
                <label className="cursor-pointer bg-[#fcfbfa] hover:bg-stone-50 border border-[#4a3b32]/20 px-3 py-2 rounded-xl text-[10px] font-black flex items-center gap-1 shadow-sm transition">
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={e => processImageUpload(e, p.id)}
                    className="hidden"
                  />
                </label>
              </div>
      ...
      <form
        ...
        className="bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-3xl p-6 shadow-md grid grid-cols-1 sm:grid-cols-3 gap-4 items-end"
      >
        <h3 className="sm:col-span-3 text-xs font-black text-[#3d2e24] border-b border-[#3d2e24]/10 pb-2 uppercase tracking-wider">
          حقن منتج طبيعي جديد في المخزن السحابي الموحد
        </h3>

        <div className="space-y-1">
          <label className="text-[10px] font-bold">اسم المنتج الفاخر:</label>
          <input
            type="text"
            required
            placeholder="الاسم"
            ...
            className="w-full bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-xl p-2.5 text-xs text-[#3d2e24] focus:outline-none"
          />
        </div>

        <div className="space-y-1">
          <label className="text-[10px] font-bold">السعر التجاري (ج.م):</label>
          <input
            type="number"
            required
            placeholder="السعر"
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-bold">الرصيد الابتدائي الحالي:</label>
          <input
            type="number"
            required
            placeholder="الكمية"
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-bold">حد التنبيه الحرج للنواقص:</label>
          <input
            type="number"
            placeholder="مثال: 5"
            value={newProductData.criticalLevel}
            onChange={e => setNewProductData({ ...newProductData, criticalLevel: e.target.value })}
            className="w-full bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-xl p-2.5 text-xs focus:outline-none"
          />
        </div>

        <div className="space-y-1">
          <label className="text-[10px] font-bold">خط انتهاء الصلاحية:</label>
          <input
            type="date"
            required
            value={newProductData.expiry}
            onChange={e => setNewProductData({ ...newProductData, expiry: e.target.value })}
            className="w-full bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-xl p-2.5 text-xs focus:outline-none"
          />
        </div>
        ...
        <div className="sm:col-span-3 space-y-1">
          <label className="text-[10px] font-bold">
            بيان وميثاق المكونات والتحاليل الشفافة للعميل النخبوي:
          </label>
          <textarea
            rows={2}
            required
            placeholder="اكتب هنا التحاليل ونسب الحموضة الفنية والمنشأ والفوائد الطبية..."
        ...
        <button
          type="submit"
          className="sm:col-span-3 bg-[#1e6b65] hover:bg-[#154d49] text-white font-black text-xs py-3 rounded-xl shadow transition"
        >
          حفظ وحقن كارت صنف المنتج الجديد بالخلفية سحابياً 🚀
        </button>
      </form>
              ...
              <tr key={p.id} className="hover:bg-stone-50">
                ...
                <td className="p-4 text-center">
                  {p.stock <= p.criticalLevel ? (
                    <span className="bg-rose-50 text-rose-600 border border-rose-200 text-[10px] px-2.5 py-1 rounded-full font-black animate-pulse">
                      تحذير: مخزون حرج!
                    </span>
                  ) : (
      ...
      <div className="bg-[#fcfbfa] border border-[#3d2e24]/10 rounded-3xl p-6 shadow-md grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <h3 className="text-xs font-black text-[#3d2e24] flex items-center gap-1.5">
            <Database className="w-4 h-4 text-[#1e6b65]" /> النسخ الاحتياطي والأمان التلقائي
          </h3>
          <p className="text-xs text-[#4a3b32]/80 leading-relaxed font-medium">
            قاعدة البيانات الاحتياطية السحابية مؤمنة كلياً وتقوم بعمل حفظ دوري وتلقائي لكافة الحركات المالية وتحديثات جرد المخازن المباشرة لـ &quot;ابن شالي&quot;.
          </p>
        </div>
        <div className="space-y-2 border-r border-[#3d2e24]/10 pr-6">
          <h3 className="text-xs font-black text-[#3d2e24] flex items-center gap-1.5">
            <Mail className="w-4 h-4 text-amber-600" /> التقارير الشاملة وجدولة الإيميل الآلي
          </h3>
          <p className="text-xs text-[#4a3b32]/80 leading-relaxed font-medium">
            النظام مجدول لإرسال تقرير وجرد أسبوعي دوري متكامل وشامل للمبيعات والمخزن مباشرة إلى بريد الإدارة المعتمد والموثق للتاجر إبراهيم:
          </p>
          <p className="text-xs font-mono font-black text-[#1e6b65] bg-[#f5f2eb] p-2 rounded-xl text-center border border-[#3d2e24]/5">
            ibrahimsiwa360@gmail.com
          </p>
        </div>
      </div>
    </div>
