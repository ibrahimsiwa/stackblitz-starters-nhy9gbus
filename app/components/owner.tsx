...
export default function Owner({ 
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto animate-fade-in pb-12">
      <div className="flex justify-between items-center border-b border-siwa-brown/10 pb-4">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white border border-siwa-brown/10 rounded-2xl p-5 shadow-sm"><span className="text-[10px] text-stone-400 font-bold uppercase block">إجمالي مبيعات الخزنة الموحدة اليوم</span><p className="text-xl font-black font-mono text-emerald-600 mt-1">{totalRevenue}.00 ج.م</p></div>
        <div className="bg-white border border-siwa-brown/10 rounded-2xl p-5 shadow-sm"><span className="text-[10px] text-stone-400 font-bold block">إجمالي مصاريف التشغيل المقيدة</span><p className="text-xl font-black font-mono text-rose-600 mt-1">{totalExpensesAmount}.00 ج.m</p></div>
        <div className="bg-white border border-siwa-brown/10 rounded-2xl p-5 shadow-sm"><span className="text-[10px] text-stone-400 font-bold block">الربح الصافي الفعلي لمعاملات اليوم</span><p className="text-xl font-black font-mono text-emerald-600 mt-1">{netProfit}.00 ج.م</p></div>
      </div>

      {/* قسم تخصيص وصيانة صور المتجر والواجهة الخلفية السيّوية المباشرة */}
      <div className="bg-white border border-siwa-brown/10 rounded-3xl p-6 shadow-md space-y-6">
        <div className="border-b border-stone-100 pb-2"><h3 className="text-xs font-black text-siwa-brown flex items-center gap-1.5">📸 إدارة وسائط المتجر وصورة الواجهة الخلفية</h3></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {products.map(p => (
              <div key={p.id} className="bg-siwa-beige/40 border border-siwa-brown/10 rounded-2xl p-4 flex justify-between items-center gap-2">
                ...
                <label className="cursor-pointer bg-white hover:bg-stone-50 border border-siwa-brown/20 px-3 py-2 rounded-xl text-[10px] font-black flex items-center gap-1 shadow-sm transition">
      ...
      <form onSubmit={submitNewProduct} className="bg-white border border-siwa-brown/10 rounded-3xl p-6 shadow-md grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
        ...
        <div className="space-y-1"><label className="text-[10px] font-bold">اسم المنتج الفاخر:</label><input type="text" required placeholder="الاسم" value={newProductData.name} onChange={(e) => setNewProductData({...newProductData, name: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs text-siwa-brown focus:outline-none" /></div>
        <div className="space-y-1"><label className="text-[10px] font-bold">السعر التجاري (ج.م):</label><input type="number" required placeholder="السعر" value={newProductData.price} onChange={(e) => setNewProductData({...newProductData, price: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs text-siwa-brown focus:outline-none" /></div>
        <div className="space-y-1"><label className="text-[10px] font-bold">الرصيد الابتدائي الحالي:</label><input type="number" required placeholder="الكمية" value={newProductData.stock} onChange={(e) => setNewProductData({...newProductData, stock: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs text-siwa-brown focus:outline-none" /></div>
        <div className="space-y-1"><label className="text-[10px] font-bold">حد التنبيه الحرج للنواقص:</label><input type="number" placeholder="مثال: 5" value={newProductData.criticalLevel} onChange={(e) => setNewProductData({...newProductData, criticalLevel: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs focus:outline-none" /></div>
        <div className="space-y-1"><label className="text-[10px] font-bold">خط انتهاء الصلاحية:</label><input type="date" required value={newProductData.expiry} onChange={(e) => setNewProductData({...newProductData, expiry: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs focus:outline-none" /></div>
        <div className="space-y-1"><label className="text-[10px] font-bold">العائلة البوتانيكية:</label>
          <select value={newProductData.category} onChange={(e) => setNewProductData({...newProductData, category: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs focus:outline-none">
        ...
        <div className="sm:col-span-3 space-y-1"><label className="text-[10px] font-bold">بيان وميثاق المكونات والتحاليل الشفافة للعميل النخبوي:</label><textarea rows={2} required placeholder="اكتب هنا التحاليل ونسب الحموضة الفنية والمنشأ والفوائد الطبية..." value={newProductData.description} onChange={(e) => setNewProductData({...newProductData, description: e.target.value})} className="w-full bg-white border border-siwa-brown/10 rounded-xl p-2.5 text-xs focus:outline-none" /></div>
        <button type="submit" className="sm:col-span-3 bg-siwa-spring hover:brightness-90 text-white font-black text-xs py-3 rounded-xl shadow transition">حفظ وحقن كارت صنف المنتج الجديد بالخلفية سحابياً 🚀</button>
      </form>
      ...
      {activeTab === 'loyalty' && (
        <div className="bg-white border border-siwa-brown/10 rounded-3xl p-6 shadow-md space-y-5 max-w-lg">
